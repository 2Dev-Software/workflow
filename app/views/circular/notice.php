                        ...
                        <div class="modal-overlay-recipient js-recipient-modal">
                            <div class="modal-container">
                                <div class="modal-header">
                                    ...
                                    <button class="modal-close js-close-recipient-modal" type="button">
                                ...
                                <div class="modal-body">
                                    <table class="recipient-table">
                                        ...
                                        <tbody class="js-recipient-table-body">
                                            <tr>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const detailModal = document.getElementById('modalNoticeSendOverlay');
        const closeDetailModalBtn = document.getElementById('closeModalNoticeSend');
        const openDetailBtns = document.querySelectorAll('.js-open-circular-send-modal');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const setupRecipients = (modalElement) => {
            if (!modalElement) return;

            const dropdown = modalElement.querySelector('.js-dropdown-content');
            const toggle = modalElement.querySelector('.js-recipient-toggle');
            const searchInput = modalElement.querySelector('.js-main-input');
            const selectAll = modalElement.querySelector('.js-select-all');
            const groupChecks = Array.from(modalElement.querySelectorAll('.group-item-checkbox'));
            const memberChecks = Array.from(modalElement.querySelectorAll('.member-checkbox'));
            const groupItems = Array.from(modalElement.querySelectorAll('.dropdown-list .item-group'));

            const recipientModal = modalElement.querySelector('.js-recipient-modal');
            ...
            groupItems.forEach((groupItem) => {
                ...
                toggleBtn?.addEventListener('click', (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    setGroupCollapsed(groupItem, !groupItem.classList.contains('is-collapsed'));
                });
            });
            ...
            const syncMemberByPid = (pid, checked, source) => {
                const normalizedPid = String(pid || '').trim();
                if (normalizedPid === '') return;
                memberChecks.forEach((m) => {
                    if (m !== source && String(m.value || '') === normalizedPid && !m.disabled) {
                        m.checked = checked;
                    }
                });
            };
            ...
            const updateSelectAllState = () => {
                ...
                groupChecks.forEach((groupCheck) => {
                    ...
                    if (memberChecked === 0) {
                        groupCheck.checked = false;
                        groupCheck.indeterminate = false;
                    } else if (memberChecked === members.length) {
                        groupCheck.checked = true;
                        groupCheck.indeterminate = false;
                    } else {
                        groupCheck.checked = false;
            ...
            groupChecks.forEach((item) => {
                item.addEventListener('change', () => {
                    getMemberChecksByGroupKey(item.getAttribute('data-group-key') || '').forEach((member) => {
                        if (!member.disabled) {
                            member.checked = item.checked;
                            syncMemberByPid(member.value || '', item.checked, member);
                        }
            ...
            btnShowRecipients?.addEventListener('click', () => {
                ...
                if (checkedGroups.length === 0 && checkedMembers.length === 0) {
                    ...
                    const recipientsMap = new Map();
                    const addRec = (pid, name, faction) => {
                        const key = String(pid || '').trim();
                        if (key && !recipientsMap.has(key)) recipientsMap.set(key, {
                            pid: key,
                            name: (name || '-').trim(),
                            faction: (faction || '-').trim()
                        });
                    ...
                    checkedGroups.forEach((item) => {
                        try {
                            JSON.parse(item.getAttribute('data-members') || '[]').forEach(m => addRec(m?.pID, m?.name, item.getAttribute('data-group-label')));
                        } catch (e) {}
                    });
                    checkedMembers.forEach((item) => addRec(item.value, item.getAttribute('data-member-name'), item.getAttribute('data-group-label')));

                    Array.from(recipientsMap.values())
                        .sort((a, b) => a.faction === b.faction ? a.name.localeCompare(b.name, 'th') : a.faction.localeCompare(b.faction, 'th'))
                        .forEach((r, i) => recipientTableBody.insertAdjacentHTML('beforeend', `<tr><td>${i+1}</td><td>${r.name}</td><td>${r.faction}</td></tr>`));
                }
                recipientModal?.classList.add('active');
            });
