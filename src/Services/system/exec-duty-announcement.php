...
$exec_duty_normalize_text = static function (string $value): string {
...
try {
    ...
    if ($stmt === false) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            $exec_duty_name = $exec_duty_normalize_text((string) ($row['fName'] ?? ''));
            $exec_duty_position = $exec_duty_normalize_text((string) ($row['positionName'] ?? ''));
